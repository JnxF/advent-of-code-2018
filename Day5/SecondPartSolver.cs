...
namespace Day5
{
    class SecondPartSolver : ISolver<int>
    {
        ...
        public int Solve()
        {
            ...
            for (char c = 'a'; c <= 'z'; ++c)
            {
                var copy = _input.Replace(c.ToString(), "").Replace(char.ToUpper(c).ToString(), "");
                var newReduced = MaxReduce(copy);
                if (newReduced.Count() < minimumLength)
                {
                    minimumLength = newReduced.Count();
                }
            }
